...
public class APILedgerAssetBinding extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(APILedgerAssetBinding.class.getName());

    public static class APILedgerAssetBindingSerializer extends StdSerializer<APILedgerAssetBinding> {
        public APILedgerAssetBindingSerializer(Class<APILedgerAssetBinding> t) {
            super(t);
        }

        public APILedgerAssetBindingSerializer() {
            this(null);
        }

        @Override
        public void serialize(APILedgerAssetBinding value, JsonGenerator jgen, SerializerProvider provider) throws IOException, JsonProcessingException {
            jgen.writeObject(value.getActualInstance());
        }
    }

    public static class APILedgerAssetBindingDeserializer extends StdDeserializer<APILedgerAssetBinding> {
        public APILedgerAssetBindingDeserializer() {
            this(APILedgerAssetBinding.class);
        }

        public APILedgerAssetBindingDeserializer(Class<?> vc) {
            super(vc);
        }

        @Override
        public APILedgerAssetBinding deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JsonProcessingException {
            ...
            if (match == 1) {
                APILedgerAssetBinding ret = new APILedgerAssetBinding();
                ret.setActualInstance(deserialized);
                return ret;
            }
            throw new IOException(String.format("Failed deserialization for APILedgerAssetBinding: %d classes match result, expected 1", match));
        }
    ...
    public Map<String, Class<?>> getSchemas() {
        return APILedgerAssetBinding.schemas;
    }
